...
async function main() {
  ...
  console.log("[test] [PROMPT 28] Séries Recentes não vive mais aqui -- migrou pra _recent-series-section.tsx (montada incondicionalmente, ver series-panel-source e _recent-series-section tests)");
  {
    assert(!/openRecentSeries|RECENT_SERIES_LIMIT|recent-series-card/.test(source), "nenhum vestígio de Séries Recentes neste arquivo -- SeriesPanel voltou a ser só create-only (Prompt 24)");
